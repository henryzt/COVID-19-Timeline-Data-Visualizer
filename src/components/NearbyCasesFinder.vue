<template>
  <div>
    ...
    <div v-if="isResult" style="text-align: center;margin-top: 20px; color: silver;">
      <a href="javascript: void(0)" v-if="currentCountry==='US'" @click="switchStateCounty">
  ...
  methods: {
    ...
    async getUSLocationData() {
      return new Promise((resolve) => {
        ...
        fetch(`https://henryz.cc/projects/covid/api_current.php`).then(
          async (res) => {
            let data = await res.json();
            ...
            this.mainLocation = json.filter(
              (ele) => ele.Country_Region === "US"
            );
