  ...
  <div class="mBlock">
    ...
    <div class="tableWrapper" v-if="tableData">
      ...
      >
        ...
        <tbody>
          <tr
            class="singleRegionData"
            v-for="singleRegion in tableData.slice(0, limit)"
            :key="singleRegion.id"
          >
            <td>
              <div class="goToCountry" v-if="tab===0" @click="switchCountry(singleRegion.location)">
    ...
    <div :class="(limit===10)?'':'stick'">
      <div
        v-if="tableData"
        class="showAll"
        @click="limit=(limit===10)?tableData.length:10"
      >{{(limit===10)?$t('table.showAll'):$t('table.showLess')}}</div>
      <SlideController
  ...
  methods: {
    ...
    updateTableData: function (currentIndex) {
      ...
      for (let region of current) {
        let lastData = last?.find(
          (element) => element.location == region.location
        );
        region.id = id++;
        sum += region[this.dataType];
        let change = lastData
          ? region[this.dataType] - lastData[this.dataType]
          : 0;
        region.change =
          (change > 0 ? "+" : "") + (this.isRate ? change.toFixed(2) : change);
      }
      if (
        sum === 0 &&
    ...
    sortByNumber: function () {
      ...
      this.tableData = [...this.tableData].sort(
        (a, b) => b[this.dataType] - a[this.dataType]
      );
    },
