  ...
  mounted() {
    ...
    this.day =
      ...
      isDateValid(
        this.mainDate,
        this.raceData[0].day,
        this.raceData[this.raceData.length - 1].day
      )
        ? this.mainDate
        : this.raceData[0].day;
    ...
    function loadData(data) {
      ...
      let daySlice = data
        .filter((d) => d.day == that.day && !isNaN(d.value))
      ...
      const getOutsideWidth = (d) =>
        x(d.value) + 15 + d.value.toFixed(0).length * 10;

      const getLabelX = (d) =>
        that.changeLabelPosition && isTooSmall(d)
          ? getOutsideWidth(d)
          : x(d.value) - 8;
      const getLabelAnchor = (d) =>
        that.changeLabelPosition && isTooSmall(d) ? "start" : "end";
      ...
      svg
        ...
        .attr("x", x(0) + 1)
        .attr("width", (d) =>
          x(d.value) - x(0) - 1 < 0 ? 0 : x(d.value) - x(0) - 1
        )
        .attr("y", (d) => y(d.rank) + 5)
      ...
      let ticker = d3.interval((e) => {
        ...
        if (that.isRate) {
          totalText.text(
            (d) =>
              "Data Average - " +
              d3.format(",.2f")(total / daySlice.length) +
              "%"
          );
        } else {
        ...
        let labels = svg.selectAll(".label").data(daySlice, (d) => d.name);
