...
const messages = {
  en: {
    ...
    launchIndicator: [
      "Stay home, stay safe.",
      "For easier access, add us to home page or bookmark.",
      "Add us to the home page or bookmark for faster access",
      "To keep you updated, bookmark or add us to the home page.",
      "Tips: tap chart legend to hide specific data set",
    ],
    subtitles: {
    ...
    nearBy: {
      ...
      ukResult: `Currently, there are<span>{1}</span> confirmed cases in<span>{0}</span>, daily increase by <span>{2}</span> cases, ranked <span>{3}</span> in all upper regions. The rate of infection per 100k people is <span>{4}</span>. There are <span>{5}</span> deaths, <span>{6}</span> new deaths today.`,
  ...
  zh: {
    ...
    nearBy: {
      ...
      ukResult: `目前，<span>{0}</span>区共有<span>{1}</span>例确诊，今日增加<span>{2}</span>例，在NHS地区中排名第<span>{3}</span>。此区域内十万人感染率为<span>{4}</span>，总死亡<span>{5}</span>例，今日新增<span>{6}</span>例。`,
